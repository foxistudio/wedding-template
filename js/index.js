...
//start spr-countdown
var $header_timer_countdown__1 = $('#header-timer-countdown--1');
$header_timer_countdown__1.countdown('2024/07/06 00:59:59', function (event) {
    $header_timer_countdown__1.find('.days').html(event.strftime('%D'));
    $header_timer_countdown__1.find('.hours').html(event.strftime('%H'));
    $header_timer_countdown__1.find('.minutes').html(event.strftime('%M'));
    $header_timer_countdown__1.find('.seconds').html(event.strftime('%S'));
...
//------------------------------------------------------------------------------------
$('#contact-form-3--0-form').validate({
    onfocusout: false,
    onkeyup: false,
    rules: {
		drink: "required",
...
$('#contact-form-3--0-form').submit(function () {
    // submit the form
    //data area
    var data = [];
    ...
    if ($(this).valid()) {
        ...
        $.ajax({
            ...
            processData: false,
            success: function () {
                $('#contact-form-3--0-form').find('[type=submit]').button('complete');
            },
            error: function () {
                $('#contact-form-3--0-form').find('[type=submit]').button('reset');
            }
        });
    } else {
